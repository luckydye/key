...
const setField = async (entry: Entry, field: string, value: string) => {
  await execkey("set", [entry.title, "--field", field, value]);
};
        ...
        if (entry.type === "entry")
          return (
            <List.Item
              ...
              subtitle={entry.user}
              detail={
                <List.Item.Detail markdown={`## ${entry.title} \n### ${entry.user}`} />
              }
              actions={
                <ActionPanel>
                  <Action
...
function NewForm() {
  return (
        ...
        <ActionPanel>
          <Action.SubmitForm
            onSubmit={async (values) => {
              if (!values.title) {
                showHUD("Title is required");
                return;
              }

              setField(values.title, "UserName", values.user).catch((e) => {
                showHUD("Error settings field");
              });

              await showHUD("Entry saved", {
                clearRootSearch: true,
                popToRootType: PopToRootType.Immediate,
              });
            }}
    ...
    >
      <Form.TextField id="title" title="Title" placeholder="Title" value={""} />
      <Form.TextField id="user" title="User" placeholder="User" value={""} />
    </Form>
  );
}
  ...
  return (
        ...
        <ActionPanel>
          <Action.SubmitForm
            onSubmit={(values) => {
              ...
              setField(entry, "UserName", user).catch((e) => {
                showHUD("Error settings field");
              });
            }}
          />
    ...
    >
      ...
      <Form.TextField
        ...
        value={entry.website}
      />
      <Form.TextArea id="notes" title="Notes" placeholder="Notes" value={entry.notes} />
      <Form.PasswordField
        id="password"
        title="Password"
